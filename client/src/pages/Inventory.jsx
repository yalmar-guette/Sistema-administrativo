...
import {
  Plus, Edit2, Trash2, Package, Search, Box,
} from 'lucide-react';
...
export default function Inventory() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setFormData({
        name: '', quantity: 0, units_per_box: 1, unit_price: 0,
      });
      loadProducts();
    } catch (error) {
...
{price.toFixed(2)}
                                ...
                                <td className="text-right font-semibold text-primary-600 dark:text-primary-400">
                                    {(price * parseFloat(exchangeRate)).toFixed(2)}
                                    {' '}
                                    Bs
</td>
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-4">
            ...
            <div className="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
              <p className="text-sm text-blue-700 dark:text-blue-300">
                ...
                unidades =
                      {' '}
                {formatQuantity(formData.quantity, formData.units_per_box)}
              </p>
            </div>
            )}
